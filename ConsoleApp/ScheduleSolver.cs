...
public class ScheduleSolver {
    ...
    private List<IterativeImprove> instances;
    ...
    public void Run() {
        ...
        Parallel.For(0, instances.Count, i => { instances[i].Run(); });
        //for (int i = 0; i < schedules.Count; i++) schedules[i].Run();

        IterativeImprove best = instances.First();
        ...
        sw.Reset();
        Plot($"{Dataname}_{Method}", Data, best.Result.order);       // make figure
        ...
        Console.WriteLine($" Time cost : {timecost:F2} seconds");
    }
